...
The `characteristics` folder contains several `.tex` files. They are organized within the folders `assumptions`, `dependencies`, `description`, `options`, `properties`, and `sources`, and are automatically generated by `create_characteristics_tex.rb`.
