...
\paragraph{Options}
The options for the HVAC Heating Type characteristic are shown in Table \ref{table:hc_opt_hvac_heating_type}. No ResStock properties are assigned based upon these options; instead the HVAC Heating Type informs other HVAC inputs (such as HVAC Heating Efficiency) that do have related ResStock properties.
...
\paragraph{Options}
The options are a combination of the specific heating systems in HVAC Heating Efficiency characteristic and the Heating Fuel characteristic. The dependency combinations are directly mapped to the options in this characteristic. There are no ResStock properties assigned directly from this input.
...
\paragraph{Options}
The options for the HVAC Heating Efficiency are ``Yes'' and ``No.'' The system that is assigned the ``Yes'' option is electric baseboard. All other systems are assigned the ``No'' option. No ResStock properties are directly assigned from this input file.
...
\end{itemize}
\paragraph{Options}
The options for HVAC Secondary Heating Type are shown in Table \ref{table:hc_opt_hvac_secondary_heating_type}. Only homes in Alaska have secondary heating, so most housing units receive a ``None'' assignment. No ResStock properties are assigned from this input file.
...
\paragraph{Options}
The options for cooling system types are shown in Table \ref{table:hc_opt_hvac_cooling_type}. The options do not assign any ResStock properties but are used as dependencies in other characteristics.
...
\paragraph{Options}
ResStock options for cooling systems are shown in Table \ref{table:hc_opt_hvac_cooling_efficiency}. If a building has a heat pump, it is assumed that the heat pump is serving both the heating and cooling load. Buildings with heat pumps assigned in the HVAC Heating characteristic are flagged, but no ResStock properties are assigned to those building based on this characteristic. Similarly, homes with a shared cooling system for the building are flagged, but the ResStock properties for that system are assigned in the HVAC Shared Systems Efficiencies characteristic.
...
\paragraph{Options}
Homes can be assigned to have shared cooling, shared heating, shared both heating and cooling, or no shared systems. No ResStock properties are assigned based on this characteristic, but these options are used as dependencies in other ResStock characteristics, such as HVAC Shared Efficiencies, where properties are assigned.
...
\paragraph{Options}
The options are either ``Yes'' or ``No.'' The options do not assign any ResStock properties.
...
None.
\paragraph{Options}
All buildings assigned an option of ``No'' with no associated ResStock properties.
\paragraph{Distribution Assumption(s)}
None.
...
None.
\paragraph{Options}
All homes currently set to ``No'' with no associated ResStock properties.
\paragraph{Distribution Assumption(s)}
None.
