...
The full year electricity bill was then calculated for each modeled dwelling unit using the dwelling unit's modeled electricity consumption, the state average variable electricity rate, and the nationwide static charge.\[Full year electricity bill =  (Fixed cost per month * 12 months)+ (Electricity consumption * Variable electricity rate)\].
...
The volumetric rate for each state and Washington D.C. was calculated as: \(\frac{(Consumption * Price) - (Fixed cost per month * 12 months * Number of consumers)}{(Consumption * Price)}\).

The rates ranged from \$0.76\/therm in North Dakota to \$4.63\/therm in Hawaii.

The full year natural gas bill was then calculated for each modeled dwelling unit using its natural gas consumption and state volumetric rate. The equation for each state is: \[Annual natural gas bill = (Fixed cost per month * 12 months) + (Natural gas consumption * Volumetric natural gas rate)\].
