...
Double & & The average miles/hour driven by the vehicle.  \\ % TODO: Find the units in OS-HPXML and change from hours to miles/hour
\end{customLongTable}
...
Then, for each electric vehicle type, we draw from Argonne National Laboratory's Autonomie20\footnote{Autonomie is the source of TEMPO's inputs about electric vehicle characteristics for both current day and future scenarios} vehicle powertrain simulation model for the electric vehicle energy consumption (\texttt{vehicle\_fuel\_economy\_combined}) and battery capacity (\texttt{vehicle\_battery\_usable\_capacity}). The average driving speed is sourced from the 2017 NHTS which summarizes U.S. travel behavior (\cite{NHTS2017}). We assume that the average speed of electric vehicles is the same as the average speed of all vehicles. 

\begin{longtable}[]{ |p{5cm}|p{3cm}|p{3cm}|p{3cm}| } \caption{Electric Vehicle Battery options and arguments that vary for each option} \label{table:hc_opt_ev_battery}  \\
  \toprule\noalign{}
  Option name & \texttt{vehicle\_battery\_usable\_capacity} & \texttt{vehicle\_fuel\_economy\_combined} & \texttt{ev\_average\_mph} \\
  \midrule\noalign{}
  \endhead
  \bottomrule\noalign{}
  \endlastfoot
  Compact, Battery Electric Vehicle, 200 mile range &  40.168 & 0.209901 & 22 \\
  Compact, Battery Electric Vehicle, 300 mile range &  63.433 & 0.220020 & 22 \\
  Midsize, Battery Electric Vehicle, 200 mile range &  41.978 & 0.219174 & 22 \\
  Midsize, Battery Electric Vehicle, 300 mile range &  65.441 & 0.229449 & 22 \\
  Pickup, Battery Electric Vehicle, 200 mile range  &  67.738 & 0.357648 & 22 \\
  Pickup, Battery Electric Vehicle, 300 mile range  & 105.946 & 0.373794 & 22 \\
  SUV, Battery Electric Vehicle, 200 mile range     &  53.503 & 0.267513 & 22 \\
  SUV, Battery Electric Vehicle, 300 mile range     &  83.680 & 0.278934 & 22 \\
\end{longtable}
