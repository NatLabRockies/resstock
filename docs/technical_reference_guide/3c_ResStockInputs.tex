...
\subsection{Modeling Approach}
The electric vehicle (EV) capabilities in ResStock were largely created under a joint project to ensure consistency between model outputs for ResStock and Transportation Energy \& Mobility Pathway Options Model\footnote{https://www.nrel.gov/transportation/tempo-model.html} (TEMPO), \cite{TEMPO}. The EV model in ResStock uses an EnergyPlus battery object to simulate the charging and discharging. Since a battery is typically associated with being attached to the building and not being driven around, the energy being discharged is ignored in the energy reporting to as the electric vehicle is assumed to only draw energy from the dwelling unit. No vehicle-to-building energy exhange is considered in the baseline model. ResStock specifies if a specific unit has an electric vehicle, the percent of time the vehicle charges at home, the vehicle type that provides the battery capacity and efficiency, the power draw of the EV charger, and the vehicle miles traveled per year. All units are sampled with percent vehicle charge at home, a vehicle type, and a vehicle miles traveled, but these options are only used in a upgrade so that these units do not need to sample these options during an upgrade. The EV battery model does take into account the degraded performance of electric vehicles at colder temperatures. The schedules of charging and discharging are stochastic. The charging and discharging schedules are based on a single occupant in the dwelling unit. The vehicle starts to discharge when the occupant leaves the unit and when returning to the unit. The duration of the discharging events are dependent on the duration at which the occupant is away, the vehicle range and efficiency, state of charge when leaving, and the outdoor temperature. Charging is assumed to start when the occupant arrives back to the dwelling unit and the battery charges until a maximum state of charge is achived or the next trip occurs. The model draws data mainly from the TEMPO model, EIA RECS 2020 (\cite{RECS2020}), the National Household Travel Survey (\cite{NHTS2017,NHTS2022}), registration data from Experian (\cite{EXPERIAN2023}), the No Place Like Home Study (\cite{NPLH2021}), and Autonomie model (\cite{AUTONOMIE}).
...
\paragraph{Options}
The options for Electric Vehicle Battery characteristic is the a set of vehicle types and battery range in miles. These options do not specifiy if the unit has a vehicle, but rather the preference of the vehicle type and battery range if the household owned an electric vehicle. Every units is assigned one of these options so that during an electric vehicle upgrade sampling to assign the vehicle type and battery range does not need to occur. The units with an electric vehicle is specified by the Electric Vehicle Ownership characteristic. The characteristic sets the \texttt{vehicle\_battery\_usable\_capacity}, \texttt{vehicle\_fuel\_economy\_combined}, and \texttt{ev\_average\_mph} ResStock arguments. The argument definitions are found in Table \ref{table:hc_arg_def_ev_battery}.
...
{Name & Required & Units & Type & Choices & Description} 
\texttt{vehicle\_batter\_usable\_capacity} & false & kWh & Double & auto & The usable capacity of the vehicle battery, only applies to electric vehicles. If not provided, the OS-HPXML default (see \href{https://openstudio-hpxml.readthedocs.io/en/v1.10.0/workflow_inputs.html#hpxml-vehicles}{HPXML Vehicles}) is used. \\
\hline
...
\paragraph{Options}
The options for Electric Vehicle Charge At Home characteristic is the a set of percentage options that determine how much of the electric vehicle's energy shows up on the residential meter vs at charing at work or public chargers. Every units is assigned one of these options so that during an electric vehicle upgrade sampling to assign the vehicle type and battery range does not need to occur. The units with an electric vehicle is specified by the Electric Vehicle Ownership characteristic. The characteristic sets the \texttt{vehicle\_fraction\_charged\_home} ResStock argument. The argument definition is found in Table \ref{table:hc_arg_def_ev_charge_at_home}.
...
\begin{itemize}
  ...
  \item Federal Poverty Level
  \item Geometry Building Type RECSgit c
  \item Tenure
  \item Vinteage ACS
\end{itemize}
...
\begin{itemize}
  ...
  \item Units reported to have an Level 1 or Level 2 charger in the field EVCHRGTYPE are assumed to have outlet access.
...
\begin{customLongTable}{|p{2.cm}|p{1.5cm}|p{1.5cm}|p{1.1cm}|p{1.4cm}|p{5.5cm}|}
  {The ResStock argument definitions set in the Electric Vehicle Miles Traveled characteristic} {table:hc_arg_def_ev_ownership} 
  {Name & Required & Units & Type & Choices & Description} 
...
Charging and discharging schedules provide a time periods when the EV can begin to charge or discharge energy. The charging and discharging schedules are connected to the stochastic occupancy model (based on American Time Use Survey data) in OpenStudio-HPXML. If a unit has an EV an occupant is chosen to be the driver. The schedule generator takes EV vehicle miles traveled and the average speed to calculate the total hours driven per year. The total hours driven per year is used as an input to the EV schedule generator and produces the charging and discharging schedule, while the battery model takes care of the actual power draw during the charging and discharging period. Figure \ref{fig:ev_discharge} shows an example of a generated discharging schedule. This schedule shows that during a “away period” for the occupant, the vehicle is discharged immediately after the occupant leaves and just before they arrive back home, proportional to the length of the away period. The charging schedule is similarly based on occupancy, where the baseline behavior assumes the vehicle is plugged in when the occupant arrives home. 
