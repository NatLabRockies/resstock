...
class UpgradeCostsTest < Minitest::Test
  def test_SFD_1story_FB_UA_GRG_MSHP_FuelTanklessWH
    cost_multipliers = {
      ...
      'Floor Area, Conditioned * Infiltration Reduction (ft^2 * Delta ACH50)' => (3 - 2) * (2250.0 * 2),
  ...
  def test_SFD_1story_FB_UA_GRG_RoomAC_ElecBoiler_FuelTanklessWH
    cost_multipliers = {
      ...
      'Floor Area, Conditioned * Infiltration Reduction (ft^2 * Delta ACH50)' => (3 - 2) * (1000.0 * 2),
  ...
  def test_SFD_1story_UB_UA_ASHP2_HPWH
    cost_multipliers = {
      ...
      'Floor Area, Conditioned * Infiltration Reduction (ft^2 * Delta ACH50)' => (3 - 2) * 3000.0,
  ...
  def test_SFD_1story_UB_UA_GRG_ACV_FuelFurnace_PortableHeater_HPWH
    cost_multipliers = {
      ...
      'Floor Area, Conditioned * Infiltration Reduction (ft^2 * Delta ACH50)' => (3 - 2) * 4500.0,
  ...
  def test_SFD_2story_CS_UA_GRG_ASHPV_FuelTanklessWH
    cost_multipliers = {
      ...
      'Floor Area, Conditioned * Infiltration Reduction (ft^2 * Delta ACH50)' => (3 - 2) * (1500.0 * 2),
  ...
  def test_SFD_2story_FB_UA_GRG_AC1_ElecBaseboard_FuelTankWH
    cost_multipliers = {
      ...
      'Floor Area, Conditioned * Infiltration Reduction (ft^2 * Delta ACH50)' => (3 - 2) * (1500.0 * 3),
  ...
  def test_SFD_2story_S_UA_GRG_ASHP1_Fireplace_FuelTanklessWH_ERV
    cost_multipliers = {
      ...
      'Size, Heat Pump Backup Primary (kBtu/h)' => 100.0, # backup
      'Size, Water Heater (gal)' => 0.0,
      'Flow Rate, Mechanical Ventilation (cfm)' => 78.7, # autosized
  ...
  def test_SFA_2story_UB_Furnace_RoomAC_FuelTankWH
    cost_multipliers = {
      ...
      'Floor Area, Conditioned * Infiltration Reduction (ft^2 * Delta ACH50)' => (3 - 2) * (250.0 * 2),
  ...
  def test_SFA_2story_FB_FuelBoiler_RoomAC_FuelTankWH
    cost_multipliers = {
      ...
      'Floor Area, Conditioned * Infiltration Reduction (ft^2 * Delta ACH50)' => (3 - 2) * (167.0 + 167.0 * 2),
  ...
  def test_MF_2story_UB_Furnace_AC1_FuelTankWH
    cost_multipliers = {
      ...
      'Floor Area, Conditioned * Infiltration Reduction (ft^2 * Delta ACH50)' => (3 - 2) * 500.0,
  ...
  def _upgrade_osw(osw)
    upgrades = { 'enclosure_ceiling' => 'R-60',
                 'enclosure_air_leakage' => '2 ACH50' }
  ...
  def _test_cost_multipliers(osw_file, cost_multipliers)
    ...
    cost_multipliers.each do |mult_type, mult_value|
      value = upgrade_costs.get_bldg_output(mult_type, hpxml_in, existing_hpxml, upgraded_hpxml)
      assert(!value.nil?)

      # Original test values used 24ft garage depth while current tests use 20ft
      # Increased the tolerance a bit to accommodate changes to surface areas.
    ...
    Dir.glob(File.join(File.dirname(__FILE__), 'results_*.*')).each { |f| File.delete(f) }
  end
end
