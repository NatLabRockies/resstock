...
def data_dictionary(df_sdr):
    ...
    df_sdr_tsagg = df_sdr[
        (df_sdr["Timeseries Publish In Full"] == "yes") & (df_sdr["Published Timeseries Name"].notna())
    ]

    # metadata_and_annual_results column names, units, and description
    df_meta = df_sdr_meta[["Published Annual Name", "Data Type", "Published Annual Unit", "Notes"]].rename(
        columns={
    ...
    df_tsagg_sdr = df_sdr_tsagg[
        ["Published Timeseries Name", "Data Type", "Published Timeseries Unit", "Notes"]
    ].rename(
        columns={
