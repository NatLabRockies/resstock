...
- Ensure you can reach the results storage specified in your workflow configuration. For `storage_backend: filesystem` the generator syncs parquet files from the referenced `s3_results_dir` into `<output_dir>/s3_data/`. For `storage_backend: minio` it reads parquet files directly from the MinIO endpoint exposed by BuildStockBatch.
...
```bash
uv run python postprocessing/resstockpostproc/upgrade_comparison/main.py --help
```
...
The tool will download parquet files from s3, prepares data for each visualization, and writes both data and the plotsto the specified output directory.

## Dashboard (Optional)

The generator produces static directories of HTML plots, SVGs, JSON, and parquet tables. Two optional Dash front ends make those artifacts easier to consume once they exist:
...
```bash
uv run python postprocessing/resstockpostproc/upgrade_comparison/dashboard/basic_dashboard.py
```
...
```bash
uv run python postprocessing/resstockpostproc/upgrade_comparison/dashboard/dynamic_dashboard.py
```
...
- Run targeted tests: `uv run pytest postprocessing/resstockpostproc/upgrade_comparison/tests`.
- Linting is configured via `ruff`; see `postprocessing/ruff.toml`.
- Install the shared hooks described in `postprocessing/.pre-commit-config.yaml` and run `pre-commit run --all-files` before opening a PR to catch formatting and spelling nits early.
- The `dashboard/` package consumes the generated artifacts and can be launched with `uv run python postprocessing/resstockpostproc/upgrade_comparison/dashboard/dynamic_dashboard.py` once outputs exist.
