...
def export_metadata_and_annual_results(raw_results_dir: str, output_dir: str, aws_profile_name=None) -> None:
    ...
    baseline_file = next(f for f in result_files if "up00" in Path(f).name.lower())
    ...
    for upgrade_id in upgrade_ids:
        ...
        processed_upgrade_df = process_simulation_outputs(
            failed_bldgs, baseline_df, processed_baseline_df, raw_upgrade_df, upgrade_id, upgrade_renamer, col_schema
        )
        cache_simulation_outputs_file(output_dir, sim_out_cache_dir, upgrade_id, processed_upgrade_df)
        up_cols = set(processed_upgrade_df.collect_schema().names())

        if upgrade_id == 0:
            processed_baseline_df = processed_upgrade_df
            base_cols = set(processed_baseline_df.collect_schema().names())
    ...
    geo_exports = [
        ...
        },
        {
            ...
            "partition_cols": {"in.state": "state"},
            "data_types": ["full"],  # TODO add basic
            "file_types": ["csv", "parquet"],
        },
    ]
    for upgrade_id in upgrade_ids:
        export_metadata_and_annual_results_for_upgrade(output_dir, upgrade_id, geo_exports)


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Process raw BuildStock results and write transformed data")
    parser.add_argument(
        "raw_results_dir",
