...
from resstockpostproc.standard_plots.schema.workflow_schema import QuantityGroup, VizType, ComparisonTypes, UpgradeInclusion, VacancyInclusion
...
QuantityType = Union[
    str,  # single column
    QuantityGroup,  # {"constituents": [...], "sum": "..."}
]
...
class PlotSpec(BaseModel):
    ...
    group_by: Optional[str] = Field(None, description="Column to facet/group by.")
    quantity: QuantityType = Field(..., description="Column(s) to visualise.")

    @field_validator("quantity")
    def _validate_quantity(cls, v, values):  # noqa: D401 – short name
    ...
    def path_segments(self, quantity_group: str) -> Path:
        ...
        path_segment /= f"Quantity Group = {quantity_group}"
