...
| **LRD 2018**                     | Load research data: publicly-available hourly residential load shapes from ~14 large electric utilities. Illustrative coverage, not nationally representative.                                                                                                                                                                                                   | 2018.                                           |

## How do these datasets differ?

Each dataset included in this dashboard represents something slightly different, and the dashboard
adjusts for some but not all of those differences. The most important ones
are below.

### Different years

The ResStock datasets use 2018 weather with ~2020 housing characteristics; the EIA and the LRD
datasets report 2018 consumption; the RECS dataset reports 2020 housing characteristics and 2020
consumption.

### Different populations

- **EIA forms** count every metered residential customer, including vacant
  and seasonally occupied homes with an active meter. For EIA
...
- **RECS** surveys _occupied primary residences_ only. For RECS comparisons,
  ResStock is filtered to occupied units.
- **LRD** is a small, non-representative set of utility service territories
  ...
  than absolute totals, and ResStock is mapped to the utility service
  territory based on the EIA-861 service territory and customer count data.

### Different methods

- **ResStock** is a bottom-up EnergyPlus simulation: every dwelling unit is
  statistically assigned a set of characteristics and then its energy
  consumption is computed from physics. The samples are then weighted up to
  a national total.
- **EIA-861, EIA-861M, and EIA-176** are broad, mandatory censuses of U.S.
  ...
  route cycle. No adjustment is made in the comparisons to account for this
  difference.
- **RECS** estimates each household's annual fuel consumption from actual
  ...
  billing data.
- **LRD** is publicly-available hourly data typically from the meters of sample
  of customers in each participating utility's service territory, normalized
  to per-dwelling-unit consumption.
