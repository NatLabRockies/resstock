...
for year in range(2012, last_year + 1):
    ...
    if year == 2012:
        ...
        col_rename_dict = {
            ...
            "MONTH": "month",
            "UTILITYID": "eiaid",
            "UTILNAME": "utility_name",
            "STATE_CODE": "state",
            "RESIDENTIAL SALES (MWh)": "sales_mwh",
            "RESIDENTIAL CUSTOMERS": "customers",
        }
        sales_df = sales_df.rename(columns=col_rename_dict)
    else:
        cols_to_read = ["Year", "Month", "Utility Number", "Utility Name", "State", "Megawatthours", "Count"]
        sales_df = pd.read_excel(
            raw_file, usecols=cols_to_read, skiprows=2, skipfooter=1, na_values=["."], sheet_name=0
        )
        col_rename_dict = {
            "Year": "year",
            "Month": "month",
    ...
    sales_df = sales_df.groupby(["year", "month", "eiaid", "state"]).agg(
