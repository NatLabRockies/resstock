...
def publish_baseline_annual_results(base_raw_df: pl.LazyFrame) -> pl.LazyFrame:
    ...
    base_df = base_df.with_columns(
        [pl.lit(0).alias("upgrade"), pl.lit("Baseline").alias("upgrade_name")]
    )
...
def publish_upgrade_annual_results(
    ...
    upgrade_df = upgrade_df.join(
        base_pub_df.select(missing_cols), on="bldg_id", how="left"
    )
...
def get_transformed_cols(df: pl.LazyFrame, col_maps: Sequence[dict]) -> pl.LazyFrame:
    ...
    for col_map in col_maps:
        if col_map["column_type"] not in ["Input", "Output"]:
            continue
        assert (
            col_map["column_name"] is not None
        ), "ResStock column name must be provided for Input or Output columns"
        if col_map["column_name"] not in all_cols:
            continue
        if col_map["conversion_factor"]:
            new_col = (
                pl.col(col_map["column_name"]).cast(pl.Float64)
                * float(col_map["conversion_factor"])
            ).alias(col_map["published_name"])
        else:
    ...
    upgrade_cols = [
        col
        for col in all_cols
        if col.startswith("upgrade_costs.") and col.endswith("_name")
    ]
...
def add_saving_cols(df: pl.LazyFrame, baseline_df: pl.LazyFrame) -> pl.LazyFrame:
    ...
    out_cols = [
        col
        for col in all_cols
        if "out." in col and not ("out.params" in col or "out.panel" in col)
    ]
    ...
    return df_with_baseline.with_columns(savings_cols).drop(
        [f"baseline_{col}" for col in out_cols]
    )
...
def add_panel_contraint_cols(df: pl.LazyFrame) -> pl.LazyFrame:
    ...
    space_constraint = (
        pl.when(pl.col(space_col) < 0).then(True).otherwise(False).alias(out_space_col)
    )
    ...
    for amp_col in amp_cols:
        ...
        amp_constraint = (
            pl.when(pl.col(amp_col) < 0).then(True).otherwise(False).alias(out_amp_col)
        )
        ...
        overall_constraint = pl.coalesce(
            pl.when(pl.col(out_amp_col) & pl.col(out_space_col)).then(
                pl.lit("Capacity and Space Constrained")
            ),
            pl.when(pl.col(out_amp_col) & ~pl.col(out_space_col)).then(
                pl.lit("Capacity Constrained Only")
            ),
            pl.when(~pl.col(out_amp_col) & pl.col(out_space_col)).then(
                pl.lit("Space Constrained Only")
            ),
...
def add_puma_column(df: pl.LazyFrame):
    ...
    pumas = gpd.read_file(
        ...
        / "resources"
        / "gisdata"
        / "ipums_pums_2010_simple_t100_area_us_puma.geojson"
    )
...
def reorder_columns(lf: pl.LazyFrame, col_maps: Sequence[dict], is_baseline: bool):
    ...
    if extra_cols:
        print(
            f"Extra columns in output data not defined in publication column definition: {extra_cols}"
        )
    missing_cols = [
        col
        for col in set(all_defined_cols) - all_df_cols
        if not col.startswith("upgrade.")
    ]
    if is_baseline:
        missing_cols = [
            col for col in missing_cols if not ("savings" in col or "reduction" in col)
        ]
    if missing_cols:
        print(
            f"Missing columns in output data that is defined in publication column definition: {missing_cols}"
        )
