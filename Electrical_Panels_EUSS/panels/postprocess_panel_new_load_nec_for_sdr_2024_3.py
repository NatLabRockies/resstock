...
!!!!!!!!!!!!!!!!!!!!!!!!!!!
THIS FILE IS REVISED BY AI USING CHANGES FROM v1 TO v2 FOR postprocess_panel_new_load_nec.py
AND IT HAS NOT BEEN TESTED. PLEASE REVIEW THE CODE CAREFULLY BEFORE RUNNING.
!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
def get_nameplate_rating(df_rating, load_category, appliance, parameter="volt-amps"):
    row = df_rating.loc[
        (df_rating["load_category"] == load_category)
        & (df_rating["appliance"] == appliance)
    ]
    return list(row[parameter])[0]


nameplate_rating = pd.read_csv("nameplate_rating_new_load.csv")
water_heater_electric_power_rating = get_nameplate_rating(
    nameplate_rating, "water heater", "electric"
)
water_heater_electric_tankless_1bath_power_rating = get_nameplate_rating(
    nameplate_rating, "water heater", "electric tankless, 1 bathroom"
)
water_heater_electric_tankless_2bath_power_rating = get_nameplate_rating(
    nameplate_rating, "water heater", "electric tankless, 2 bathrooms"
)
water_heater_electric_tankless_3bath_power_rating = get_nameplate_rating(
    nameplate_rating, "water heater", "electric tankless, 3+ bathrooms"
)
water_heater_heat_pump_power_rating = get_nameplate_rating(
    nameplate_rating, "water heater", "heat pump"
)
water_heater_heat_pump_120_power_rating = get_nameplate_rating(
    nameplate_rating, "water heater", "heat pump, 120V, shared"
)

washer_power_rating = get_nameplate_rating(
    nameplate_rating, "clothes washer", "electric"
)
dryer_elctric_ventless_power_rating = get_nameplate_rating(
    nameplate_rating, "clothes dryer", "electric ventless"
)
dryer_elctric_power_rating = get_nameplate_rating(
    nameplate_rating, "clothes dryer", "electric"
)
dryer_elctric_120_power_rating = get_nameplate_rating(
    nameplate_rating, "clothes dryer", "electric, 120V"
)
dryer_heat_pump_power_rating = get_nameplate_rating(
    nameplate_rating, "clothes dryer", "heat pump"
)
dryer_heat_pump_120_power_rating = get_nameplate_rating(
    nameplate_rating, "clothes dryer", "heat pump, 120V"
)

range_elctric_power_rating = get_nameplate_rating(
    nameplate_rating, "range/oven", "electric"
)
range_induction_power_rating = get_nameplate_rating(
    nameplate_rating, "range/oven", "induction"
)
range_elctric_120_power_rating = get_nameplate_rating(
    nameplate_rating, "range/oven", "electric, 120V"
)
range_induction_120_power_rating = get_nameplate_rating(
    nameplate_rating, "range/oven", "induction, 120V"
)

dishwasher_power_rating = get_nameplate_rating(
    nameplate_rating, "dishwasher", "standard"
)
hot_tub_spa_power_rating = get_nameplate_rating(
    nameplate_rating, "hot tub/spa", "electric"
)
pool_heater_power_rating = get_nameplate_rating(
    nameplate_rating, "pool heater", "electric, 27 kW"
)
pool_pump_1_and_half_hp_power_rating = get_nameplate_rating(
    nameplate_rating, "pool pump", "1.5 HP"
)
pool_pump_2_hp_power_rating = get_nameplate_rating(
    nameplate_rating, "pool pump", "2 HP"
)

well_pump_1_hp_power_rating = get_nameplate_rating(
    nameplate_rating, "well pump", "1 HP"
)
well_pump_1_and_half_hp_power_rating = get_nameplate_rating(
    nameplate_rating, "well pump", "1.5 HP"
)
ventilation_kitchen_power_rating = get_nameplate_rating(
    nameplate_rating, "ventilation", "kitchen, 300 cfm"
)
ventilation_bathroom_power_rating = get_nameplate_rating(
    nameplate_rating, "ventilation", "bathroom, 50 cfm"
)

EVSE_power_rating_level1 = get_nameplate_rating(
    nameplate_rating, "electric vehicle charger", "electric, level 1"
)
EVSE_power_rating_level2 = get_nameplate_rating(
    nameplate_rating, "electric vehicle charger", "electric, level 2"
)
garbage_disposal_three_quarters_hp_power_rating = get_nameplate_rating(
    nameplate_rating, "garbage disposal", "3/4 HP"
)
garage_door_half_hp_power_rating = get_nameplate_rating(
    nameplate_rating, "garage door opener", "1/2 HP"
)
...
def _fixed_load_water_heater(row, check_completed_status):
    ...
    if (row[f"{bxm}.water_heater_in_unit"] == "Yes") and (
        (row[f"{bxm}.water_heater_fuel"] == "Electricity")
        or ("Electric" in row[f"{bxm}.water_heater_efficiency"])
    ):
        ...
        if "Heat Pump" in row[f"{bxm}.water_heater_efficiency"]:
            if (
                "120V" in row[f"{bxm}.water_heater_efficiency"]
                or "120 V" in row[f"{bxm}.water_heater_efficiency"]
            ):
...
def _special_load_dryer(row, method, check_completed_status):
    ...
    if (
        "Electric" not in row[f"{bxm}.clothes_dryer"]
        or row[f"{bxm}.clothes_dryer"] == "None"
    ):
        return 0

    if "Heat Pump" in row[f"{bxm}.clothes_dryer"]:
        if (
            "120V" in row[f"{bxm}.clothes_dryer"]
            or "120 V" in row[f"{bxm}.clothes_dryer"]
        ):
            if method == "83":
                return 0
            if method == "87":
...
def _special_load_cooking_range_oven(row, check_completed_status):
    ...
    if (
        "Electric" not in row[f"{bxm}.cooking_range"]
        or row[f"{bxm}.cooking_range"] == "None"
    ):
        return 0

    if "Induction" in row[f"{bxm}.cooking_range"]:
        if (
            "120V" in row[f"{bxm}.cooking_range"]
            or "120 V" in row[f"{bxm}.cooking_range"]
        ):
...
def _special_load_space_heating_no_ahu(row, check_completed_status):
    ...
    secondary_heating_type = get_heating_type(
        row[f"{bxm}.hvac_secondary_heating_efficiency"]
    )
    ...
    system_cols = [
        ...
        "er" if row[f"{upgc}.size_heat_pump_backup_primary_k_btu_h"] > 0 else None,
    ]
...
def _special_load_space_cooling_no_ahu(row, check_completed_status):
    ...
    cooling_type = get_cooling_type(row[f"{bxm}.hvac_cooling_efficiency"])
    cooling_load = hvac_cooling_conversion(
        row[f"{upgc}.size_cooling_system_primary_k_btu_h"], system_type=cooling_type
    )
...
def _special_load_space_conditioning(row, check_completed_status):
    ...
    Returns:
        (heating_load, cooling_load, max(loads), error_msg)
    """
    if check_completed_status and row["completed_status"] != "Success":
        return np.nan, np.nan, np.nan, None

    heating_types, heating_loads = _special_load_space_heating_no_ahu(row, check_completed_status)
    cooling_type, cooling_load = _special_load_space_cooling_no_ahu(row, check_completed_status)
...
def _new_load_space_conditioning(row, option_columns, check_completed_status):
    ...
    if "hp" in (heating_type or ""):
        if new_heating and "existing" in row[upg_name].lower():
            backup_heating_type = get_heating_type(
                row[f"{bxm}.hvac_heating_efficiency"]
            )
        else:
...
def _new_load_evse_oedi(row, option_columns, method):
    for opt_col in option_columns:
        if (
            opt_col in ["upgrade.electric_vehicle", "upgrade.electric_vehicle_charger"]
            and row[opt_col]
        ):
...
def _get_air_handlers(row, heating_types, cooling_type, apply_check=True) -> tuple[float, float, Optional[str]]:
    ...
    if _is_heat_pump(heating_type):
        if _is_ducted(secondary_heating_type):
            ...
            heat_ahu, msg = _get_air_handler_for_heating(row[f"{upgc}.size_heating_system_secondary_k_btu_h"], secondary_heating_type, apply_check=apply_check)
            if msg:
            ...
            if _is_ducted(heating_type):
                if _is_fuel(backup_heating_type) and _is_ducted(backup_heating_type):
                    ...
                    heat_ahu, msg = _get_air_handler_for_heating(row[f"{upgc}.size_heat_pump_backup_primary_k_btu_h"], backup_heating_type, apply_check=apply_check)
                    if msg:
        ...
        # not heat pump
        heat_ahu1, msg = _get_air_handler_for_heating(row[f"{upgc}.size_heating_system_primary_k_btu_h"], heating_type, apply_check=apply_check)
        if msg:
            error_msg += f"Primary non-HP heating error: {msg} "

        heat_ahu2, msg = _get_air_handler_for_heating(row[f"{upgc}.size_heating_system_secondary_k_btu_h"], secondary_heating_type, apply_check=apply_check)
        if msg:
...
def hvac_240V_air_handler(nom_cap) -> float:
    ahu_volt = get_nameplate_rating(
        nameplate_rating,
        "space heating/cooling",
        "electric air handler",
        parameter="voltage",
    )
    amp_para = get_nameplate_rating(
        nameplate_rating,
        "space heating/cooling",
        "electric air handler",
        parameter="amperage",
    ...
    ahu_amp = max(
        float(amp_para[2]), float(amp_para[0]) * float(nom_cap) + float(amp_para[1])
    )
...
def hvac_120V_air_handler(nom_cap) -> float:
    ahu_volt = get_nameplate_rating(
        nameplate_rating, "space heating", "fuel air handler", parameter="voltage"
    )
    amp_para = get_nameplate_rating(
        nameplate_rating, "space heating", "fuel air handler", parameter="amperage"
    ).split(",")
    ahu_amp = max(
        float(amp_para[2]), float(amp_para[0]) * float(nom_cap) + float(amp_para[1])
    )
...
def apply_va_linear_regression(nom_cap, load_category, appliance) -> float:
    volt = get_nameplate_rating(
        nameplate_rating, load_category, appliance, parameter="voltage"
    )
    amp_para = get_nameplate_rating(
        nameplate_rating, load_category, appliance, parameter="amperage"
    ).split(",")
...
def standard_amperage(x: float) -> int:
    ...
    # TODO: refine
    standard_sizes = np.array([50, 100, 125, 150, 200, 225, 250])
...
def read_file(
    filename: str, low_memory: bool = True, sort_bldg_id: bool = False, **kwargs
) -> pd.DataFrame:
...
def apply_existing_loads_limited(
    row, method: str, check_completed_status: bool
) -> list:
    """breakdown of existing load for limited load categories"""

    existing_loads = list(_special_load_space_conditioning(row, check_completed_status))  # (heating_load, cooling_load, hvac_load, error_msg)
    existing_loads += [
...
def apply_existing_loads(
    row, method: str, check_completed_status: bool, n_kit: int = 2, n_ldr: int = 1
) -> list:
    """Load summing method"""

    existing_loads = apply_existing_loads_limited(row, method, check_completed_status)
    existing_loads += [
        _general_load_lighting(row, check_completed_status),  # sqft
        _general_load_kitchen(
            row, check_completed_status, n=n_kit
        ),  # consider logic based on sqft
        _general_load_laundry(
            row, check_completed_status, n=n_ldr
        ),  # consider logic based on sqft (up to 2)
...
def apply_demand_factor(x, threshold=8000):
    ...
    Split load into the following tiers and apply associated multiplier factor
        If threshold == 8000:
            ...
            > 8kVA : 0.4
    """
    return 1 * min(threshold, x) + 0.4 * max(0, x - threshold)
...
def calculate_new_loads(
    ...
    HC_list = [
        ...
        f"{bxm}.hvac_has_shared_system",
        f"{bxm}.hvac_heating_efficiency",
        f"{bxm}.hvac_cooling_efficiency",
        f"{bxm}.hvac_secondary_heating_efficiency",
    ]
    ...
    option_columns = [
        x for x in dfu.columns if x.startswith(f"{upgc}.option") and x.endswith("name")
    ]
    option_cols = []
    upgrade_options = []
    for opt_col in option_columns:
        upgrade_option = [
            x for x in dfu[opt_col].unique() if x != "" and not pd.isna(x)
        ]
        if upgrade_option:
            assert (
                len(upgrade_option) == 1
            ), f"{upgrade_option=} has more than one option."
    ...
    df_up[["hvac_eq_changed", "new_load_heating", "new_load_cooling", "new_load_hvac", "new_hvac_msg"]] = df_up.apply(
        lambda x: _new_load_space_conditioning(x, option_cols, check_completed_status),
        axis=1, result_type="expand",
    )

    # [2] Water heating
    wh_option_cols, _ = get_upgrade_columns_and_options(
        option_cols, upgrade_options, "Water Heater Efficiency"
    )
    df_up["new_load_water_heater"] = df_up.apply(
        lambda x: _new_load_water_heating(x, wh_option_cols, check_completed_status),
        axis=1,
    )

    # [3] Appliances
    dryer_option_cols, _ = get_upgrade_columns_and_options(
        option_cols, upgrade_options, "Clothes Dryer"
    )
    df_up["new_load_dryer"] = df_up.apply(
        lambda x: _new_load_dryer(x, dryer_option_cols, method, check_completed_status),
        axis=1,
    )

    cooking_option_cols, _ = get_upgrade_columns_and_options(
        option_cols, upgrade_options, "Cooking Range"
    )
    df_up["new_load_range_oven"] = df_up.apply(
        lambda x: _new_load_range_oven(x, cooking_option_cols, check_completed_status),
        axis=1,
    )

    hot_tub_spa_option_cols, _ = get_upgrade_columns_and_options(
        option_cols, upgrade_options, "Misc Hot Tub Spa"
    )
    df_up["new_load_hot_tub_spa"] = df_up.apply(
        lambda x: _new_load_hot_tub_spa(
            x, hot_tub_spa_option_cols, check_completed_status
        ),
        axis=1,
    )

    pool_heater_option_cols, _ = get_upgrade_columns_and_options(
        option_cols, upgrade_options, "Misc Pool Heater"
    )
    df_up["new_load_pool_heater"] = df_up.apply(
        lambda x: _new_load_pool_heater(
            x, pool_heater_option_cols, check_completed_status
        ),
        axis=1,
    )

    ev_option_cols, _ = get_upgrade_columns_and_options(
        option_cols, upgrade_options, "Electric Vehicle"
    )
    if not ev_option_cols:
        ev_option_cols, _ = get_upgrade_columns_and_options(
            option_cols, upgrade_options, "Electric Vehicle Charger"
        )
    df_up["new_load_evse"] = df_up.apply(
        lambda x: _new_load_evse(x, ev_option_cols, method, check_completed_status),
        axis=1,
    )
    ...
    if result_as_map:
        df_up = df_up[[bldg_id, "hvac_eq_changed", "new_hvac_msg"] + new_load_cols]
    else:
...
def calculate_new_loads_oedi(
    df: pd.DataFrame, dfu: pd.DataFrame, method: str, result_as_map: bool = False
) -> pd.DataFrame:
    ## apply new load
    # 1 add necessary baseline HC
    HC_list = [
    ...
    df_up[["hvac_eq_changed", "new_load_heating", "new_load_cooling", "new_load_hvac", "new_hvac_msg"]] = df_up.apply(
        lambda x: _new_load_space_conditioning_oedi(x, option_cols), axis=1, result_type="expand"
    )

    # [2] Water heating
    df_up["new_load_water_heater"] = df_up.apply(
        lambda x: _new_load_water_heating_oedi(x, option_cols), axis=1
    )

    # [3] Appliances
    df_up["new_load_dryer"] = df_up.apply(
        lambda x: _new_load_dryer_oedi(x, option_cols, method), axis=1
    )
    df_up["new_load_range_oven"] = df_up.apply(
        lambda x: _new_load_range_oven_oedi(x, option_cols), axis=1
    )
    df_up["new_load_hot_tub_spa"] = df_up.apply(
        lambda x: _new_load_hot_tub_spa_oedi(x, option_cols), axis=1
    )
    df_up["new_load_pool_heater"] = df_up.apply(
        lambda x: _new_load_pool_heater_oedi(x, option_cols), axis=1
    )
    df_up["new_load_evse"] = df_up.apply(
        lambda x: _new_load_evse_oedi(x, option_cols, method), axis=1
    )
...
def get_upgrade_columns_and_options(option_columns, upgrade_options, parameter):
    ...
    options = (
        []
    )  # e.g., 'ASHP, SEER 15, 9.0 HSPF' if parameter = 'HVAC Heating Efficiency'
...
### -------- load method calcs --------
def calculate_new_load_total_220_83(
    dfi: pd.DataFrame,
    dfu: pd.DataFrame,
    n_kit: int = 2,
    n_ldr: int = 1,
    ...
    if oedi_data_format:
        ...
        df_new = calculate_new_loads(
            df, dfu, "83", check_completed_status, result_as_map=True
        )
    ...
    df_existing = pd.DataFrame(
        df.apply(
            lambda x: apply_existing_loads(
                x, "83", check_completed_status, n_kit=n_kit, n_ldr=n_ldr
            ),
            axis=1,
        ).to_list(),
        index=df.index,
        columns=existing_loads_detailed,
    )
    ...
    df_existing[total_load_pre] = df_existing[existing_loads].apply(
        lambda x: apply_total_load_220_83(x, has_new_hvac_load=False), axis=1
    )
    ...
    df_upgraded = df_new[new_loads].rename(
        columns=dict(zip(new_loads, upgradable_loads))
    )
    ...
    loads_upgraded = cond_upgraded.apply(lambda x: list(x[x].index), axis=1).rename(
        "loads_upgraded_83"
    )  # record which loads are accounted as new in final calculation
    ...
    df_loads.loc[new_hvac, total_load_post] = df_loads.loc[new_hvac].apply(
        lambda x: apply_total_load_220_83(x, has_new_hvac_load=True), axis=1
    )
    df_loads.loc[~new_hvac, total_load_post] = df_loads.loc[~new_hvac].apply(
        lambda x: apply_total_load_220_83(x, has_new_hvac_load=False), axis=1
    )
    ...
    return dfu.join(
        df_result[cols].drop(columns=[upg_name]).set_index(bldg_id), on=bldg_id
    )


def calculate_new_load_total_220_87(
    df: pd.DataFrame,
    dfu: pd.DataFrame,
    explode_result: bool = False,
    result_as_map: bool = False,
    ...
    df_existing = pd.DataFrame(
        df.apply(
            lambda x: apply_existing_loads_limited(x, "87", check_completed_status), axis=1
        ).to_list(),
    ...
    if oedi_data_format:
        ...
        df_new = calculate_new_loads(
            df, dfu, "87", check_completed_status, result_as_map=True
        )
    ...
    df_new = df[cols].join(df_new.set_index([bldg_id]), on=[bldg_id], how="right")
    ...
    df_new[total_load_post] = (
        df_new[total_load_pre] + df_new["new_load_total_VA_220_87"]
    )
...
def main(
    ...
    if explode_result:
        ext = "_exploded"
    if result_as_map:
        output_filename = output_filedir / (
            upgrade_filename.stem.split(".")[0]
            + f"__res_map__nec_new_load{ext}"
            + upgrade_filename.suffix
        )
    else:
        output_filename = output_filedir / (
            upgrade_filename.stem.split(".")[0]
            + f"__nec_new_load{ext}"
            + upgrade_filename.suffix
        )

    upgrade_num = [
        x
        for x in list(chain(*[x.split(".") for x in upgrade_filename.stem.split("_")]))
        if "up" in x
    ][0]
    plot_dir_name = f"plots_sfd" if sfd_only else f"plots"
    output_dir = (
        upgrade_filename.parent / plot_dir_name / "nec_calculations" / upgrade_num
    )
    ...
    if plot:
        if not output_filename.exists():
            ...
            generate_plots(
                df, dfo, output_dir, sfd_only=sfd_only, upgrade_num=upgrade_num
            )
            sys.exit()

    if oedi_data_format and (
        baseline_filename.suffix != ".parquet" or upgrade_filename.suffix != ".parquet"
    ):
        msg = "For OEDI data format, only parquet files are accepted."
        raise ValueError(msg)

    df = read_file(
        baseline_filename, compression="infer", low_memory=False, sort_bldg_id=True
    )
    dfu = read_file(
        upgrade_filename, compression="infer", low_memory=False, sort_bldg_id=True
    )
    ...
    if building_id:
        dfu = dfu.loc[dfu[bldg_id]==int(building_id)].reset_index(drop=True)
        df = df.loc[df[bldg_id]==int(building_id)].reset_index(drop=True)
    ...
    if bldgs_no_bl:
        print(
            f"WARNING: {len(bldgs_no_bl)} buildings have upgrade but no baseline: {bldgs_no_bl}"
        )
    ...
    assert (
        dfu[bldg_id] == df[bldg_id]
    ).prod() == 1, "Ordering of building_id does not match between upgrade and baseline"
    ...
    if not garbage_disposal_assignment_file.exists():
        ...
        df[[bldg_id, "has_garbage_disposal"]].to_csv(garbage_disposal_assignment_file, index=False)  # save assignment for reference and reproducibility
    else:
        df_gd = pd.read_csv(garbage_disposal_assignment_file)
        gd_id_col = bldg_id if bldg_id in df_gd.columns else "building_id"
        assert set(df[bldg_id]).issubset(set(df_gd[gd_id_col])), "Garbage disposal assignment file does not match baseline building_id"
        df_gd = df_gd.rename(columns={gd_id_col: bldg_id})
        df = df.merge(df_gd[[bldg_id, "has_garbage_disposal"]], on=bldg_id, how="left")
    ...
    if result_as_map:
        df1 = calculate_new_load_total_220_83(
            df,
            dfu,
            n_kit=2,
            n_ldr=1,
            explode_result=explode_result,
            result_as_map=result_as_map,
            oedi_data_format=oedi_data_format,
        )
        df2 = calculate_new_load_total_220_87(
            df,
            dfu,
            explode_result=explode_result,
            result_as_map=result_as_map,
        ...
        dfu1 = calculate_new_load_total_220_83(
            df,
            dfu,
            n_kit=2,
            n_ldr=1,
        ...
        dfo = calculate_new_load_total_220_87(
            df, dfu1, explode_result=explode_result, oedi_data_format=oedi_data_format
        )
